...
export const softKeyFor = (loop, offset) => {
    if (loop === -1)
        return 0x52;
    const ret = (loop + offset) + 0x52;
    if (ret > 0x56 || ret < 0x53) {
        return null;
    } return ret;
};
console.log(softKeyFor(-1, 0), 0x52);
console.log(softKeyFor(-1, -3), 0x52);
console.log(softKeyFor(0, -1), 0x52);
