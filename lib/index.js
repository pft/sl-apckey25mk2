...
const cycleDeviceMode = (store) => {
  ...
  store.dispatch(deviceAction("levels", 0));
  store.dispatch(
    deviceAction("pan", autoLatch.feed(BUTTONS.BTN_KNOB_CTRL_PAN, EV_NOTE_OFF))
  );
  set_syncs = autoLatch.feed(BUTTONS.BTN_KNOB_CTRL_SEND, EV_NOTE_OFF);
...
const cycleLevelMode = (store) => {
  ...
  if (levelMode !== 0) {
    store.dispatch(deviceAction("mode", -1));
    store.dispatch(
      deviceAction(
        "pan",
...
const onMIDIMessage = (store) => (_deltaTime, message) => {
  ...
  if (evtype === EV_NOTE_ON || evtype === EV_NOTE_OFF) {
    if (button === BUTTONS.BTN_KNOB_CTRL_PAN) {
      ...
      dispatch(deviceAction("levels", 0));
      return;
    }
