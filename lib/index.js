...
connect(output, 'APC Key 25 mk2:APC Key 25 mk2 APC Key 25 mk2 C 20:1', "SooperLooperAkaiAPCKey25mk2Controller");
connect(input, 'APC Key 25 mk2:APC Key 25 mk2 APC Key 25 mk2 C 20:1', "SooperLooperAkaiAPCKey25mk2Controller");
...
const onMIDIMessage = (_deltaTime, message) => {
  ...
  if (evtype === EV_NOTE_ON || evtype === EV_NOTE_OFF) {
    if (button === BUTTONS.BTN_KNOB_CTRL_VOLUME) {
      store.dispatch(deviceAction("volume", evtype === EV_NOTE_ON));
      return;
    }
    if (button === BUTTONS.BTN_KNOB_CTRL_PAN) {
      store.dispatch(deviceAction("pan", evtype === EV_NOTE_ON));
      return;
    }
...
const createAllPads = (storeState) => {
  ...
  const pads = [0, 1, 2, 3, 4].flatMap((y) => {
    ...
    const cellLedModeFn = trackLedModeFn(track);
