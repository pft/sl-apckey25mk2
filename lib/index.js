...
function reducer(state = { tracks: [] }, action) {
  switch (action.type) {
    ...
    case "batch":
      return R.reduce(reducer, state, action.value);
    default:
      return state;
  }
}
...
const batchAction = (actions) => ({ type: "batch", value: actions });
...
const levelModes = [undefined, "all", "selected"];
...
const getLoops = (session) => {
  const body = {
    address: "/loops",
    args: [
      { type: "s", value: OSC_HOST },
      { type: "i", value: OSC_PORT },
      { type: "s", value: session },
      { type: "s", value: "/loops" },
    ],
  };
  udpPort.send(body, OSC_SL_HOST, 1234);
...
const cycleDeviceMode = (store) => {
  ...
  store.dispatch(
    batchAction([
      deviceAction(
        "sync",
        autoLatch.feed(BUTTONS.BTN_KNOB_CTRL_SEND, EV_NOTE_OFF),
      ),
  ...
  unregister_selected(["in_peak_meter"]);
};

const cycleLevelMode = (store) => {
  const state = store.getState();
  let levelMode = getDeviceSetting("levels")(state) || 0;
  levelMode = (levelMode + 1) % levelModes.length;
  if (levelMode !== 0) {
    if (levelMode === 2) register_selected(["in_peak_meter"]);
    store.dispatch(
      batchAction([
        deviceAction(
          "sync",
        ...
        deviceAction("levels", levelMode),
      ]),
    );
  } else {
    store.dispatch(deviceAction("levels", levelMode));
    unregister_selected(["in_peak_meter"]);
  }
};
...
const onMIDIMessage = (store) => (_deltaTime, message) => {
  ...
  const set_syncs = syncMode(storeState);
  if (evtype === EV_CC) {
    const delta = knobsEase.feed(button, message[2], shifted);
    if (delta === undefined) return;
    const knobnum = message[1] - 48;
    if (showTrackLevels(storeState)) {
      if (knobnum === 0) return;
      const level = track_levels[knobnum];
      if (!level) return;
      const loopnum = getGlob("selected_loop_num")(storeState);
      if (loopnum === -1) return;
  ...
  if (
    ...
    if (getDeviceSetting("pan")(storeState)) return;
    const pad = button;
    ...
    if (set_syncs) {
      ...
      if (
        show8ths &&
        (button < 30 || button > 31 || (button > 31 && button < 40))
      ) {
        const setting = "eighth_per_cycle";
    ...
    if (evtype === EV_NOTE_ON) {
      ...
      if (button < 40 && showTrackLevels(storeState)) {
        let value = (5 - row) / 5;
        const trackno = getGlob("selected_loop_num")(storeState);
        const isglob = trackno === -1;
        if (isglob && (numpad < 2 || numpad > 4)) return;
        const levelTrack = isglob ? storeState.glob : tracks[trackno];
        if (!levelTrack) return;
        const ctrl = track_levels[button % 8];
        const storedValue = levelTrack[ctrl];
        if (Math.round(storedValue * 5) === Math.round(value * 5))
          value -= 1 / 10;
        if (Math.round(storedValue * 100) == 10 && row === 4) value = 0;
        if (isglob) {
          dispatch(globAction(ctrl, value));
          justSend(`/set`, { type: "s", value: ctrl }, { type: "f", value });
        } else {
          dispatch(trackAction(levelTrack, ctrl, value));
          justSend(
            `/sl/${trackno}/set`,
            { type: "s", value: ctrl },
            { type: "f", value },
      ...
      if (button < 32 && R.path(["device", "levels"])(storeState)) {
      ...
      if (R.path(["device", "levels"])(storeState)) {
        const setting = "wet";
        let value = (numpad + 1) / 8;
      ...
      if (numpad === 0) {
        ...
        if (
          R.propOr(SL_STATE_UNKNOWN, "state", stateTrack) <
            SL_STATE_RECORDING || // Off or undefined
          (!force_alt1 && R.prop("state", stateTrack) === SL_STATE_RECORDING) || // Recording sans force => play
          (force_alt1 && R.prop("state", stateTrack) !== SL_STATE_RECORDING) // Not recording, having data, but forced
        ) {
      ...
      if (numpad === 7) {
        if (shifted)
          justSend(
            `/sl/${track}/set`,
            { type: "s", value: "delay_trigger" },
            { type: "f", value: Math.random() },
          );
        else justSend(`/sl/${track}/hit`, { type: "s", value: "pause" });
      }
      return;
    }
  }
  if (evtype === EV_NOTE_ON || evtype === EV_NOTE_OFF) {
    if (button === BUTTONS.BTN_KNOB_CTRL_VOLUME && evtype === EV_NOTE_ON) {
      cycleLevelMode(store);
      return;
    }
    if (button === BUTTONS.BTN_KNOB_CTRL_PAN) {
      dispatch(
        batchAction([
          ...
          deviceAction("pan", autoLatch.feed(button, evtype)),
          deviceAction("levels", 0),
        ]),
      );
      unregister_selected(["in_peak_meter"]);
    ...
    if (button === BUTTONS.BTN_KNOB_CTRL_SEND) {
      dispatch(
        batchAction([
          deviceAction("sync", autoLatch.feed(button, evtype)),
          deviceAction(
            "pan",
            autoLatch.feed(BUTTONS.BTN_KNOB_CTRL_PAN, EV_NOTE_OFF),
          ),
    ...
    // if (evtype === EV_NOTE_ON && getDeviceSetting("levels")(storeState)) {
    ...
    if (
      shifted &&
      button >= BUTTONS.BTN_SOFT_KEY_CLIP_STOP &&
      button <= BUTTONS.BTN_SOFT_KEY_SELECT
    ) {
      const row = button - 0x52;
      const track = row === 0 ? -1 : shiftedTrack(row, loopoffset);
      if (track < loopcount) {
    ...
    if (evtype == EV_NOTE_ON && button == BUTTONS.BTN_STOP_ALL_CLIPS) {
      if (shifted)
        justSend("/sl/-1/hit", ("s", "mute_off"));
      else justSend("/sl/-1/hit", ("s", "mute_on"));
      return;
    }
...
const register_selected = (ctrls) => {
  ctrls.forEach((ctrl) => {
    requestFeedback(
      `/sl/-3/register_auto_update`,
      "/update",
      {
        type: "s",
        value: ctrl,
...
const unregister_selected = (ctrls) => {
  ctrls.forEach((ctrl) => {
    requestFeedback(`/sl/-3/unregister_auto_update`, "/update", {
      type: "s",
      value: ctrl,
    });
  });
...
const trackCommands = [2, 6, 7, 8, 13, 12, 16, 14];
...
const syncMode = R.path(["device", "sync"]);

const showTrackLevels = R.both(
  R.compose(R.lt(1), getDeviceSetting("levels")),
  R.compose(R.lt(-2), getGlob("selected_loop_num")),
);
  ...
  [
    ...
    (state) => (track, i) => {
      return (xpad) => {
        ...
        const tracknum = getGlob("selected_loop_num")(state);
        const theTrack =
          (tracknum === -1 ? state.glob : state.tracks[tracknum]) || {};
...
const createAllPads = (storeState) => {
  ...
  const loopoffset = getLoopoffset(storeState);
  const set_syncs = syncMode(storeState);
  const ctrl_keys = R.range(0x40, 0x48).flatMap((x) => {
    if (x === BUTTONS.BTN_KNOB_CTRL_VOLUME) {
      const levelmode = R.either(
        getDeviceSetting("levels"),
        () => 0,
      )(storeState);
      const ret = [0x90, x, levelmode];
      return ret;
    }
    if (x === BUTTONS.BTN_KNOB_CTRL_PAN) {
      const ret = [0x90, x, Number(getDeviceSetting("pan")(storeState) || 0)];
      return ret;
    }
    if (x === BUTTONS.BTN_KNOB_CTRL_SEND) {
      const ret = [
        0x90,
        x,
        (set_syncs && 1) || 0, // Number(getDeviceSetting("set_syncs")(storeState) || 0)
      ];
      return ret;
    }
    if (x === BUTTONS.BTN_KNOB_CTRL_DEVICE) {
      const devicemode = R.compose(
        R.max(0),
        R.either(getDeviceMode, () => 0),
      )(storeState);
      const ret = [0x90, x, devicemode];
      return ret;
    }
    return [];
  });
  if (devicemode > 0) {
    ...
    return overlay(sessionnums, emptycells).concat(ctrl_keys);
  }
  const tracks = R.propOr([], "tracks")(storeState);
  const toprow =
    set_syncs || showTrackLevels(storeState)
      ? []
  ...
  const matrix = [0, 1, 2, 3, 4].flatMap((y) => {
    ...
    //      console.log(track);
    if (!(set_syncs || showTrackLevels(storeState)) && y === 0) {
      return toprow;
    }
  ...
  const soft_keys = [0, 1, 2, 3, 4].flatMap((y) => {
    ...
    return [
      0x90,
      0x52 + y,
      tracknum === getGlob("selected_loop_num")(storeState)
        ? showTrackLevels(storeState)
          ? 0x02
          : 0x01
        : 0,
    ];
  ...
  const eighths = show8ths
    ...
    : [];
  const pads = matrix.concat(overlay(soft_keys, ctrl_keys));
  if (pads.length) {
    if (shifted) {
...
function debounce(func, timeout = 32) {
  let timer;
  return (...args) => {
    clearTimeout(timer);
    timer = setTimeout(() => {
...
subscribe(store, debounce(renderChangedPads, 32));
...
udpPort.on("message", function (oscMsg, timeTag, info) {
  ...
  if (oscMsg.address === "/pong") {
    ...
    justSend(
      "/set",
      { type: "s", value: "smart_eighths" }, // Undocumented, auto 8th in slgui
      { type: "f", value: 0 },
    );
    return;
  }
  ...
  if (oscMsg.address === "/update") {
    ...
    if (ctrl === "in_peak_meter") {
      store.dispatch({
        type: "track",
        track: getGlob("selected_loop_num")(store.getState()),
        ctrl,
        value: val * 2,
      });
