...
import { KnobSpeedControl, ButtonAutoLatch } from "./helpers.js";
import {
  BUTTONS,
  SL_STATES,
  LED_BRIGHTS,
...
const onMIDIMessage = (store) => (_deltaTime, message) => {
  ...
  if (
    ...
    if (set_syncs) {
      ...
      if (numpad === 5) {
        ...
        justSend(
          `/sl/${track}/set`,
          { type: "s", value: "overdub_quantized" },
          { type: "f", value: Number(!stateTrack["overdub_quantized"]) }
        );
      }
      if (numpad === 6) {
        // True per-loop setting
        justSend(
          `/sl/${track}/set`,
          { type: "s", value: "replace_quantized" },
          { type: "f", value: Number(!stateTrack["replace_quantized"]) }
        );
      }
      return;
    }
    ...
    if (numpad === 0) {
      if (track === -1) {
        justSend(`/sl/${track}/hit`, { type: "s", value: "record_or_overdub" });
        return;
      }
      if (
  ...
  if (evtype === EV_NOTE_ON || evtype === EV_NOTE_OFF) {
    ...
    // if (button === BUTTONS.BTN_PLAY) {
    //   dopause = evtype === EV_NOTE_ON;
    //   return;
    // }
