...
let domute = false;
let loopoffset = 1;
let devicemodes = ["loops", "sessionload", "sessionsave"];
let devicemode = 0;
...
const onMIDIMessage = (_deltaTime, message) => {
  ...
  if (
    ...
    const pad = button;
    const track = shiftedTrack(padRow(pad), loopoffset);
    const numpad = pad % 8;
    const stateTrack = R.path(["tracks", track], store.getState());
    if (devicemodes[devicemode] === "sessionload") {
      const uri = `${SL_SESSION_PATH}${String(button).padStart(2, "0")}.sless`;
      justSend(
        `/load_session`,
        { type: "s", value: uri },
        {
...
const unregister_update = () => {
  ctrls.forEach((ctrl) => {
    requestFeedback(`/sl/[0-99]/unregister_update`, "/update", {
      type: "s",
      value: ctrl,
    });
  });
  auto_ctrls.forEach((ctrl) => {
    requestFeedback(`/sl/[0-99]/unregister_auto_update`, "/update", {
      type: "s",
      value: ctrl,
    });
  });
...
const renderAllPads = () => {
      ...
      [0, 1, 2, 3, 4].flatMap((y) => {
        return rowPads(y).flatMap((pad, x) => {
          const cell = [
            LED_BRIGHT_100,
            pad,
            devicemode === 1 ? COLOR_DARK_GREEN : COLOR_ORANGE,
          ];
          return cell;
        });
      })
  ...
  const pads = [0, 1, 2, 3, 4].flatMap((y) => {
    ...
    const track = store.getState().tracks[tracknum] || {};
    const {
      ...
      if (set_syncs) {
        return rowPads(y).flatMap((pad, x) => [
          ...
          settingcolors[x] instanceof Array
            ? settingcolors[x][track[settings[x]]]
            : settingcolors[x],
        ]);
      }
...
udpPort.on("message", function (oscMsg, timeTag, info) {
  ...
  if (oscMsg.address === "/info") {
    ...
    if (loopcount > previousCount) {
      register_update(loopcount - 1);
      //      console.log("loop:", previousCount);
      justSend(
        `/sl/${previousCount}/set`,
        { type: "s", value: "sync" },
        { type: "f", value: 1 }
      );
    }
    if (loopcount < previousCount) {
      store.dispatch({ type: "empty-track", value: previousCount - 1 });
    }
    return;
  }
