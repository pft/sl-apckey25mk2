...
import { KnobSpeedControl, ButtonAutoLatch, cycle } from "./helpers.js";
import {
  BUTTONS,
  SL_STATES,
  LED_BRIGHTS,
...
const padBrightnessForLevel = (num, level) => {
  ...
  return (x) =>
    x < roundedpos ? LED_BRIGHT_100 : x === roundedpos ? last : LED_BRIGHT_25;
};
...
const createAllPads = (storeState) => {
  ...
  const eighths = show8ths
    ...
    : [];
  const pads = matrix.concat(soft_keys);
  if (pads.length) {
    if (shifted) {
